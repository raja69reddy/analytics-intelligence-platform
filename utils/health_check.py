    ...
    try:
        ...
        for vw in views:
            try:
                ...
                items.append(
                    HealthItem(f"View: {vw}", True, f"{len(df.columns)} columns")
                )
            except Exception as exc:
...
def _check_dashboard_pages() -> list[HealthItem]:
    ...
    for pg in expected:
        ...
        exists = path.exists()
        items.append(
            HealthItem(f"Dashboard: {pg}", exists, "exists" if exists else "missing")
        )
    return items
...
def print_report(result: dict) -> None:
    ...
    print(f"\n{sep}")
    print(
        f"  OVERALL SCORE: {result['passed']}/{result['total']} checks passed  ({score}/100)"
    )
    if score == 100:
