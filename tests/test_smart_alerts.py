...
def _make_conversion_df(n: int = 30, drop: bool = False) -> pd.DataFrame:
    ...
    return pd.DataFrame({"session_date": dates, "sessions": sessions, "goal_completions": conv})
...
def test_alert_to_dict_has_required_keys():
    ...
    d = a.to_dict()
    required = {"alert_id", "alert_type", "severity", "title", "message",
                "recommended_action", "metric_value", "threshold_value", "detected_at"}
...
def test_generate_alert_message_returns_string(monkeypatch):
    ...
    import importlib, ai.smart_alerts.detector as det
