...
def test_dim_dates_date_range():
    ...
    df = query_df(
        "SELECT MIN(full_date)::TEXT mn, MAX(full_date)::TEXT mx FROM dim_dates"
    )
    assert df["mn"].iloc[0] == "2023-01-01"
    # dim_dates now extends to cover 2026 raw data
    assert (
        df["mx"].iloc[0] >= "2026-01-01"
...
def test_generate_summary_runs():
    ...
    assert "SUMMARY REPORT" in result.stdout, "Expected 'SUMMARY REPORT' in output"
